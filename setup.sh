#!/bin/bash

#Declare colors.
txtred='\e[0;31m' # Red
txtgrn='\e[0;32m' # Green
txtblu='\e[0;34m' # Blue
#echo -e "${txtred}Setting up your environment..."

MYSQL_SIGNATURE="-----BEGIN PGP PUBLIC KEY BLOCK-----
Version: SKS 1.1.6
Comment: Hostname: pgp.mit.edu


sTSKHe+QnnnoFmu4gnmDU31i
=Xqbo
-----END PGP PUBLIC KEY BLOCK-----"


if [[ ! -f "./signature.key" ]]; 
    echo "$MYSQL_SIGNATURE" > ./signature.key;
fi
echo -e "${txtblu}Installing mysql...";
sudo apt-key add signature.key
sudo sh -c 'echo "deb http://repo.mysql.com/apt/ubuntu bionic mysql-5.7" >> /etc/apt/sources.list.d/mysql.list'
sudo apt update
sudo apt install -f mysql-client=5.7* mysql-community-server=5.7* mysql-server=5.7*
echo -e "${txtgrn} Done installing mysql✅"

echo -e "${txtblu}Setting up your python environment...";
python -m venv venv;
source ./venv/bin/activate;
sudo apt install -y libmysqlclient-dev;
sudo apt install -y python3-lxml
echo -e "${txtgrn} Done setting up your python environment✅"

echo -e "${txtblue}Checking if you have a requirements.txt file...";
if [[ -f "./requirements.txt" ]]; then
    echo -e "${txtblu}Installing requirements...";
    pip install -r ./requirements.txt;
    echo -e "${txtgrn} Done installing python packages✅"
else
    echo -e "${txtred}You need to have a requirements file";
    exit 1;
fi

echo -e "${txtblu}Starting mysql service and setting up the database...";
sudo service mysql start;
if [[ -f "./setup_mysql_dev.sql" ]]; then
    mysql -u root -p < "./setup_mysql_dev.sql" 
    echo -e "${txtgrn} Done setting up your database✅"
else
    echo -e "${txtred}You need a setup file for your database.";
    echo -e "${txtred}It should be named setup_mysql_dev.sql.";
    exit 1;
fi
